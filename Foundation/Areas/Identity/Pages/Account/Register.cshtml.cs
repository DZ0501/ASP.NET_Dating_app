...
namespace Foundation.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private FoundationUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<FoundationUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(FoundationUser)}'. " +
                    $"Ensure that '{nameof(FoundationUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<FoundationUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<FoundationUser>)_userStore;
        }
    }
}
